...
class Emitter(object):
    ...
    def printer(self, read_filename, write_filename):
        ...
        for line in fp_read:
            ...
            lst = [((part, search_word), self.emit(part, search_word)) for part in self.word_to_part[search_word]]
            ...
            part = pair[0]
            fp_write.write("{0} {1} {2}\n".format(word, part, math.log(self.emit(part, search_word))))
...
if __name__ == "__main__":
    emitter = Emitter()
    emitter.classify("ner.counts_rare")
    emitter.printer("ner_dev_fixed.dat", "prediction.file")
